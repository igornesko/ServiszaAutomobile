...
def unesiLice(*args): #funkvija za unos podataka u treeview
        ...
        if ime=="" or marka=="" or status=="" or popravka=="" or deo=="" or cd=="" or cp=="":
            ...
            try:
                    int(cd)
                    int(cp)
                    tree.insert("" , 'end', values=(ime.title(),marka.title(), status.title(), popravka.title(), deo.title(), cd, cp))
...
def potvrda(tree, entryIme, entryMarka, entryStatus, entryPopravka, entryDeo, entryCD, entryCP):  #funkcija (vezana za Edit) koja ubacuje u tree izmejnene vrednosti
    ...
    tree.insert('',indeks, values=(entryIme, entryMarka, entryStatus, entryPopravka, entryDeo, int(entryCD), int(entryCP)))
    ...
    try:
        ...
        def memorisiIZatvori(): #ubacuje izmenjene vrednosti u tree
            try:
                int(edit6.get())
                int(edit7.get())
                if potvrda(tree, edit1.get(), edit2.get(), edit3.get(), edit4.get(), edit5.get(), str(edit6.get()), str(edit7.get())):
                    editWin.destroy()
            except ValueError:
                    messagebox.showinfo("Greska", "Telefon mora biti broj!")
                    edituj()
        def cancel():
            editWin.destroy()
